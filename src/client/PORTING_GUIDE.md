...
- **`astro.config.mjs` is loaded in its own module graph**, separate from the SSR graph the content loaders run in. A module-level variable shared between an integration and a loader exists twice, so state an integration writes is invisible to the loaders. Put that state on `globalThis` (see `__cncUmbracoTree` in `src/lib/umbraco.ts`).
