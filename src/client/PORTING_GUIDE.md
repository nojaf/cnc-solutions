...
Open findings from the full review of 6 September 2026, most important first. Already fixed and therefore not listed: the GitHub Actions deploy, the cookie banner reading its copy from Umbraco, and the Turnstile script placement.

1. **Tablet container widths (576 to 767px).** `News.astro`, `NewsPage.astro`, `Team.astro`, `Hero.astro`, `HomeCarousel.astro`, `HomeRow.astro`, `HomeSolutions.astro` and `404.astro` use `md:max-w-180` without `sm:max-w-135`. Measured with `compare` at 600 and 700px: news articles are 600/700 wide instead of 540, team columns 568/668 instead of 510, home-row headings 284/334 instead of 255. `AboutRow.astro` is correct (Gatsby used `container-md` there) and must keep its current caps. Re-verify with `--selector article --pages /nieuws/`, `--selector h3 --pages /team/` and `--selector h2 --pages /` at 600 and 700.
2. **Home row download button** (`HomeRow.astro`, the `fileDownloadFile` branch). Links the raw backend URL instead of `mediaFileHref()`, and renders a plain `<a class="btn-cnc">`, which has no styling because that rule is scoped to `ButtonCnc.astro`. Use `ButtonCnc` with `download`, like `Product.astro`. No home row uses the field today, so it only shows once an editor fills it in.
3. **Footer has no navigation-text fallback** (`Footer.astro`). `Navigation.astro` falls back to the node name with `navText()`; the footer renders an empty heading when the optional `navigationText` is blank. Share the helper.
4. **Bracket lengths that break the hard rule in step 2.** `text-[30px]` (nine uses) is exactly `text-3xl`. `max-w-[350px] md:max-w-[500px]` in `HomeSolutions.astro` is a plain length. The font sizes with no scale step (`lg:text-[22px]`, `lg:text-[25px]`, `sm:text-[32px]`, `lg:text-[40px]`) need a decision: either add a font-size exemption to the rule, or accept the drift and note it.
5. **The container row in the Bootstrap mapping table contradicts the Lessons.** It still says `mx-auto max-w-240 lg:max-w-240 xl:max-w-285`; the lesson says `sm:max-w-135 md:max-w-180 lg:max-w-240 xl:max-w-285`. Fix the table, since future ports copy it.
6. **Solution rows keep one media and one text block** (`Solution.astro`, the `rows` mapping). Gatsby rendered every block in a row. Only matters if content ever has two media blocks in one row.
7. **Undocumented improvement.** The previous-news link on `NewsPage.astro` points at the nearest older article; Gatsby pointed at the oldest (`news-page.js` used `find` on an ascending list). Add a line to the Lessons.
8. **Cosmetic.** Bracket colours `text-[#212529]` (Contact), `text-[rgb(108,117,125)]` (News) and `hover:text-[#6b9636]` (ProductCard) could become theme tokens. `Navigation.astro` takes an unused `currentPageId` prop. `astro check` shows one harmless hint on `Analytics.astro` for `gtagId` inside `define:vars`.
